...
function renderUsers() {
  ...
  [...usersList].sort((a, b) => (b.role === 'owner') - (a.role === 'owner') || a.name.localeCompare(b.name, 'tr')).forEach((u) => {
    ...
    applyName(nm, u.name + (u.id === me.id ? ' (sen)' : '') + (u.role === 'owner' ? ' 👑' : '') + (u.profile && u.profile.level ? ` · Lv${u.profile.level}` : '') + (u.profile && u.profile.pet ? ' ' + u.profile.pet.e : ''), u.profile);
...
function buildMessage(msg, prev) {
  ...
  if (!grouped) {
    ...
    applyName(nm, msg.name, msg.profile || (usersList.find((x) => x.id === msg.uid) || {}).profile, ((msg.profile && msg.profile.title) ? ' • ' + msg.profile.title : '') + ((msg.profile && msg.profile.pet) ? ' ' + msg.profile.pet.e : ''));
...
buildLogin(); setupComposer(); setupVoiceButtons(); setupSoundboard(); setupSovs(); setupWatch(); setupMobile(); setupLogout(); setupHelp(); setupSearch(); setupUIButtons(); setupAfk(); connect();
