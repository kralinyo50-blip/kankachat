...
function openUI() {
  ...
  UI_OPTS.forEach(([key, title, opts]) => {
    ...
    opts.forEach(([val, label]) => {
      ...
      b.className = 'ui-opt' + (uiCfg[key] === val ? ' sel' : '');
      b.textContent = label;
      b.onclick = () => {
        uiCfg[key] = val;
...
function renderUsers() {
  ...
  [...usersList].sort((a, b) => (b.role === 'owner') - (a.role === 'owner') || a.name.localeCompare(b.name, 'tr')).forEach((u) => {
    ...
    applyName(nm, u.name + (u.id === me.id ? ' (sen)' : '') + (u.role === 'owner' ? ' 👑' : '') + (u.profile && u.profile.level ? ` · Lv${u.profile.level}` : ''), u.profile);
...
function buildMessage(msg, prev) {
  ...
  if (!grouped) {
    ...
    applyName(nm, msg.name, msg.profile || (usersList.find((x) => x.id === msg.uid) || {}).profile, (msg.profile && msg.profile.title) ? ' • ' + msg.profile.title : '');
...
const CLIENT_ACH = { first_msg: '💬 İlk Mesaj', chatty: '🗣️ 100 Mesaj', rich100: '🪙 Cüzdan 100', rich1000: '💰 Cüzdan 1000', lvl5: '📈 Seviye 5', gamer: '🏆 İlk Zafer', social: '📨 İlk DM', ai_friend: '🤖 AI Dostu' };
...
function renderProfile() {
  ...
  const SECTIONS = [['emoji', '😎 Avatar Emojisi'], ['frame', '🖼️ Çerçeveler'], ['badge', '🎖️ Rozetler'], ['namecolor', ' İsim Renkleri'], ['title', '🎗️ Ünvanlar'], ['status', '💬 Özel']];
...
buildLogin(); setupComposer(); setupVoiceButtons(); setupSoundboard(); setupMobile(); setupLogout(); setupHelp(); setupSearch(); setupUIButtons(); connect();
