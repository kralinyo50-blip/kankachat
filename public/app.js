...
let pendingImage = null, lastTypingSent = 0, metaInfo = { requireInvite: false, inviteCode: null };
let shop = [], myCoins = 0;
...
function setupComposer() {
  ...
  $('#fileInput').addEventListener('change', (e) => {
    ...
    if (!f) return;
    if (!f.type.startsWith('image/')) { alert('Şimdilik sadece resim 🖼️'); return; }
    if (f.size > 1_200_000) { alert('Resim çok büyük (max ~1 MB)'); return; }
    const r = new FileReader();
    r.onload = () => { pendingImage = r.result; $('#imgPreviewImg').src = pendingImage; $('#imgPreview').classList.remove('hidden'); };
    r.readAsDataURL(f);
  });
...
function clearPendingImage() { pendingImage = null; $('#imgPreview').classList.add('hidden'); $('#imgPreviewImg').src = ''; }
...
function renderProfile() {
  ...
  tip.textContent = 'Coin kazan: !maden ⛏️ !yarisma 🧠 !xo 🕹️ !zincir 🔗 !bahis 🎰 + her gün girişte +10 🪙';
...
buildLogin(); setupComposer(); setupVoiceButtons(); setupSoundboard(); setupMobile(); setupLogout(); connect();
